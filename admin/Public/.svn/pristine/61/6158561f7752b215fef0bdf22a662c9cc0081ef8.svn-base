...
$(function (){
    ...
    // $.get("/Public/skin-config.html", function (data) {
...
function saveMenuState(state){
    if(localStorageSupport){
        localStorage.setItem("collapse_menu",state);
    }
}
...
function getLocalTime(nS) { 
    return new Date(parseInt(nS) * 1000).toLocaleString().replace(/年|月/g, "-").replace(/日/g, " ").replace(/\//g,"-");
}
